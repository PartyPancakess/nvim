        ...
        function()
          ...
          if #lsp_clients > 0 then
            for _, client in ipairs(lsp_clients) do
              ...
              table.insert(lsp_details, string.format("  %s %s %s", format_icon, client.name, supports_formatting and "(supports formatting)" or "(no formatting)"))
            end
          end
          ...
          table.insert(sections, string.format("📊 Type: %s | Size: %s | Lines: %d", filetype, size_str, buf_info.lines))
          table.insert(sections, string.format("⚙️  Encoding: %s | Format: %s | Indent: %s", buf_info.encoding, buf_info.format, buf_info.indent))
